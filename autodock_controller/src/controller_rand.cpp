...
void autodock_controller::centering_state_fun(){
    ...
    if ((in_view) and (fabs(pose_set.theta - desire_angle)<0.2)) {
        openrover_stop();
        set_docking_state("approach");
    }
    else{
        openrover_turn(default_turn*sign(pose_set.theta - desire_angle));
    }
...
void autodock_controller::final_approach_state_fun(){
    ...
    if (fabs(tag_yaw)>pose_set.theta_bounds){
        openrover_stop();
...
void autodock_controller::action_state_manage(){
    ...
    if (docking_state == "centering"){
        if ((in_view) and (fabs(pose_set.theta - desire_angle)<0.1)) {
            openrover_stop();
            set_docking_state("approach");
    ...
    if (tag_y){
        if ((fabs(tag_y)<0.05) and (desire_angle == tune_angle)){
            openrover_stop();
            desire_angle = 0;
            ROS_INFO("final stop");
        }
